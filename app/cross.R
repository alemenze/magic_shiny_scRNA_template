observe({
    ...
    updateSelectizeInput(session, "CCCluster1", choices=c(unique(levels(data2$data[[input$CCSeuratObject]]$seurat_clusters))), server=TRUE, options = list(maxOptions = 50))   
    updateSelectizeInput(session, "CCCluster2", choices=c(unique(levels(data2$data[[input$CCSeuratObject]]$seurat_clusters))), server=TRUE, options = list(maxOptions = 50)) 
})
